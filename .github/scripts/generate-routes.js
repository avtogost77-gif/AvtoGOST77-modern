...
// База городов России (ТОП-100 для быстрого старта)
const CITIES = {
  "moskva": {
    ...
    population: 12500000,
    isHub: true,
    priority: ["spb", "kazan", "voronezh", "samara", "nizhniy-novgorod", "ekaterinburg", "rostov", "chelyabinsk"]
  },
  "spb": {
    ...
    population: 5400000,
    isHub: true,
    priority: ["moskva", "kaliningrad", "murmansk", "arkhangelsk"]
  },
  "kazan": {
  ...
  "voronezh": {
    ...
    population: 1060000,
    priority: ["moskva", "rostov", "kursk", "belgorod"]
  },
  "samara": {
    ...
    population: 1150000,
    priority: ["moskva", "kazan", "ufa", "saratov"]
  },
  "nizhniy-novgorod": {
    ...
    population: 1250000,
    priority: ["moskva", "kazan", "yaroslavl", "vladimir"]
  },
  "ekaterinburg": {
    ...
    population: 1500000,
    priority: ["moskva", "chelyabinsk", "perm", "ufa"]
  },
  "rostov": {
    name: "Ростова-на-Дону", nameTo: "Ростов-на-Дону", 
    region: "Ростовская область",
    coords: [47.2357, 39.7015],
    population: 1140000,
    priority: ["moskva", "voronezh", "krasnodar", "volgograd"]
  },
  "chelyabinsk": {
    name: "Челябинска", nameTo: "Челябинск",
    region: "Челябинская область",
    coords: [55.1644, 61.4368], 
    population: 1200000,
    priority: ["ekaterinburg", "ufa", "kurgan"]
  },
  "ufa": {
    name: "Уфы", nameTo: "Уфу",
    ...
    population: 1130000,
    priority: ["moskva", "kazan", "samara", "ekaterinburg"]
  }
};

// Ключевые концепции из проекта АвтоГОСТ
...
// Расчет расстояния между городами (упрощенная формула)
...
function generateRouteContent(fromCity, toCity, distance, price) {
  ...
  const uniqueContent = `
    ...
    <section class="route-calculator">
      <div class="container">
        <h2>Калькулятор стоимости ${from.name} — ${to.nameTo}</h2>
        <div id="route-calculator" data-from="${fromCity}" data-to="${toCity}" data-distance="${distance}">
          <!-- Здесь будет встроен калькулятор -->
        </div>
      </div>
    </section>
        ...
        <div class="content-grid">
          <div class="content-main">
            ...
            <blockquote class="brand-quote">
              <p>"${BRAND_CONCEPTS.whileYou}"</p>
              <cite>— ${BRAND_CONCEPTS.slogan}</cite>
            </blockquote>
          </div>
          ...
          <div class="content-sidebar">
            <div class="contact-card">
              ...
              <a href="tel:+79162720932" class="btn btn-primary">+7 916 272-09-32</a>
              <a href="https://wa.me/79162720932" class="btn btn-whatsapp">WhatsApp</a>
            </div>
...
async function generateRoutes() {
  ...
  console.log(`🚀 Генерируем ${pagesCount} маршрутных страниц...`);
  ...
  for (const [fromCityCode, fromCity] of Object.entries(CITIES)) {
    ...
    await fs.ensureDir(`routes/${fromCityCode}`);
...
<body>
    ...
    <ul>
        ${fromCity.priority ? fromCity.priority.map(toCityCode => {
          ...
          if (toCity) {
            return `<li><a href="${fromCityCode}-${toCityCode}.html">Грузоперевозки ${fromCity.name} — ${toCity.nameTo}</a></li>`;
          }
          return '';
        }).join('') : ''}
    </ul>
    ...
    await fs.writeFile(`routes/${fromCityCode}/index.html`, cityIndexContent);
  ...
  console.log(`🎉 Сгенерировано ${generatedCount} маршрутных страниц!`);
}
