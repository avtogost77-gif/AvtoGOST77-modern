...
### **ВАЖНО! КОЭФФИЦИЕНТЫ ТИПОВ ТС:**
```yaml
Фура 20т:    ×1.0   (100%) - базовая цена
10-тонник:   ×0.86  (86%)  - дешевле фуры
5-тонник:    ×0.71  (71%)  - еще дешевле
3-тонник:    ×0.46  (46%)  - значительно дешевле
Газель:      ×0.36  (36%)  - самая дешевая
```

**Принцип:** Чем меньше ТС, тем дешевле! Коэффициенты применяются к итоговой цене.
...
### **СИСТЕМА ПЛЕЧ:**
```yaml
Короткое плечо (200-300км):  180₽/км
Среднее плечо (300-800км):   120₽/км
Длинное плечо (800+км):      85₽/км
```

### **ФОРМУЛА:**
```
Базовая цена = Расстояние × Тариф плеча
Проверка минималки = max(Базовая цена, Минималка ТС регион)
Сборный груз = Цена × 0.65 (если применимо)
Коэффициент ТС = применяется к итоговой цене (фура ×1.0, газель ×0.36)
...
6. **Применение коэффициентов** загрузки, маршрута, груза
